...
use lumina_part1_codegen::{compile_to_entry, write_object_file};
...
pub fn build_project(project_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let expr = parse(&source).map_err(|e| format!("{e:?}"))?;
    let typed = TypeChecker::new().check(&expr).map_err(|e| format!("{e:?}"))?;
    let ir = compile_to_entry(&typed).map_err(|e| e.to_string())?;
