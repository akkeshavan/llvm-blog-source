//! Lum CLI - init, build, run for Lumina projects
...
fn link_executable(main_obj: &Path, runtime_obj: &Path, out_exe: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    link.arg(main_obj).arg(runtime_obj);
    if cfg!(target_os = "macos") && std::path::Path::new("/opt/homebrew/lib/libgc.dylib").exists() {
        link.arg("/opt/homebrew/lib/libgc.dylib");
    } else if cfg!(target_os = "macos") {
        link.arg("-L/opt/homebrew/lib").arg("-lgc");
    } else {
        link.arg("-lgc");
    }
    let status = link.arg("-o").arg(out_exe).status()?;
    if !status.success() {
...
pub fn build_project(project_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let prog = parse(&source).map_err(|e| format!("{e:?}"))?;
    let mut tc = TypeChecker::new();
    let typed = tc.check_program(&prog).map_err(|e| format!("{e:?}"))?;
    ...
    println!("Build succeeded: {}", exe_path.display());
    Ok(exe_path)
}
...
mod tests {
    ...
    fn build_produces_ir() {
        if !have_tool("clang") {
            // Skip on machines without LLVM tools / clang on PATH.
            return;
        }
        let dir = tempfile::tempdir().unwrap();
        init_project(&dir.path().to_path_buf()).unwrap();
