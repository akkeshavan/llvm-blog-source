...
int64_t lumina_array_i64_new(void) {
    lumina_arr_i64 *a = (lumina_arr_i64*)malloc(sizeof(lumina_arr_i64));
...
int64_t lumina_array_str_new(void) {
    lumina_arr_str *a = (lumina_arr_str*)malloc(sizeof(lumina_arr_str));
    a->cap = LUMINA_ARRAY_INIT_CAP;
    a->len = 0;
    a->data = (const char**)malloc((size_t)a->cap * sizeof(const char*));
    return (int64_t)(uintptr_t)a;
}
void lumina_array_str_append(int64_t h, const char* s) {
    ...
    if (a->len >= a->cap) {
        a->cap *= 2;
        a->data = (const char**)realloc(a->data, (size_t)a->cap * sizeof(const char*));
    }
    a->data[a->len++] = s;
}
